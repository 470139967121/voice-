...
sealed class UiText {
    ...
    fun resolve(): String = when (this) {
        is Res -> if (args.isEmpty()) stringResource(resource)
        else stringResource(resource, *args.toTypedArray())
        is Plain -> text
    }

    /** Resolve inside a suspend scope (e.g. LaunchedEffect). */
    suspend fun resolveAsync(): String = when (this) {
        is Res -> if (args.isEmpty()) getString(resource)
        else getString(resource, *args.toTypedArray())
        is Plain -> text
    }

    companion object {
        fun res(resource: StringResource, vararg args: Any): UiText = Res(resource, args.toList())
