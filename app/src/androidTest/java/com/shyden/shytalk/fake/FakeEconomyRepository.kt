...
class FakeEconomyRepository : EconomyRepository {
    ...
    override suspend fun pullGacha(pullCount: Int, expectedCost: Int): Resource<GachaResult> =
        Resource.Success(GachaResult(coinsSpent = expectedCost, newBalance = balance.value - expectedCost))

    override suspend fun sendGift(recipientId: String, giftId: String, quantity: Int): Resource<Map<String, Any?>> =
        Resource.Success(mapOf("success" to true, "giftName" to "Gift", "quantity" to quantity))
