...
class FirebaseCallTest {
    ...
    @Test
    fun `firebaseCall returns Success when block succeeds`() = runTest {
    ...
    @Test
    fun `firebaseCall returns Success with Unit for side-effect block`() = runTest {
        val result = firebaseCall { /* side effect */ }
        assertTrue(result is Resource.Success)
    }

    @Test
    fun `firebaseCall returns Error with exception message`() = runTest {
    ...
    @Test
    fun `firebaseCall returns Error with fallback message when exception message is null`() = runTest {
        val result = firebaseCall<String>("Custom error") {
    ...
    @Test
    fun `firebaseCall uses default fallback message`() = runTest {
        val result = firebaseCall<String> {
            throw RuntimeException(null as String?)
        }
        assertTrue(result is Resource.Error)
    ...
    @Test
    fun `firebaseCall handles IllegalStateException`() = runTest {
