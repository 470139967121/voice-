...
class FunFactSplashViewModelTest {
    ...
    private fun makeFunFact(id: String) = FunFact(
        id = id,
        text = "Fact $id",
        category = "trivia",
        emoji = "🤔",
    ...
    private fun createViewModel() = FunFactSplashViewModel(
    ...
    @Test
    fun `warmUpComplete becomes true after all jobs finish`() = runTest {
    ...
    @Test
    fun `funFacts populated from repository on init`() = runTest {
    ...
    @Test
    fun `warmUpComplete true even if bannerRepository throws`() = runTest {
    ...
    @Test
    fun `warmUpComplete true even if funFactRepository throws`() = runTest {
    ...
    @Test
    fun `warmUpComplete true when currentUserId is null`() = runTest {
    ...
    @Test
    fun `funFacts starts with cached facts before sync completes`() = runTest {
    ...
    @Test
    fun `synced fun facts replace cached facts after warm-up`() = runTest {
        val cached = listOf(makeFunFact("cached-1"))
    ...
    @Test
    fun `funFacts keeps cached value when sync returns empty list`() = runTest {
    ...
    @Test
    fun `banners with URL action type are fetched without error`() = runTest {
        val urlBanner = Banner(
            id = "banner-1",
            title = "Sale",
