...
class IdentityRepositoryImplTest {
    ...
    @Test
    fun `resolveIdentity returns Found with uniqueId for existing identity`() = runTest {
    ...
    @Test
    fun `resolveIdentity returns NotFound when identity not in system`() = runTest {
        val response = JSONObject().apply {
            put("found", false)
        }
        coEvery { workerApiClient.post("/api/users/sign-in", any()) } returns response
    ...
    @Test
    fun `resolveIdentity returns Deactivated for unlinked identity`() = runTest {
        val response = JSONObject().apply {
            put("found", false)
    ...
    @Test
    fun `resolveIdentity sends correct provider and identifier in body`() = runTest {
    ...
    @Test
    fun `resolveIdentity returns Error on network failure`() = runTest {
    ...
    @Test
    fun `createUser returns uniqueId on success`() = runTest {
        val response = JSONObject().apply {
            put("success", true)
            put("created", true)
    ...
    @Test
    fun `createUser sends all fields in request body`() = runTest {
        val response = JSONObject().apply {
            put("success", true)
    ...
    @Test
    fun `createUser returns Error on 409 conflict`() = runTest {
    ...
    @Test
    fun `linkProvider calls correct endpoint`() = runTest {
    ...
    @Test
    fun `linkProvider sends provider and identifier in body`() = runTest {
        val response = JSONObject().apply { put("success", true) }
    ...
    @Test
    fun `unlinkProvider calls correct endpoint with DELETE`() = runTest {
        val response = JSONObject().apply { put("success", true) }
        var capturedPath: String? = null
        var capturedBody: JSONObject? = null
    ...
    @Test
    fun `forceRefreshToken clears api client token cache`() = runTest {
