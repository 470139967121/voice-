...
test.describe('Admin Panel', () => {
  ...
  test('has email and password inputs', async ({ page }) => {
    const emailInput = page.locator('#login-screen input[type="email"], #login-screen input[placeholder*="mail" i]');
  ...
  test('has Firebase config loaded', async ({ page }) => {
    const firebaseConfig = await page.evaluate(() => (window as any).SHYTALK_CONFIG?.FIREBASE_CONFIG);
  ...
  test('dashboard has all expected tab buttons', async ({ page }) => {
    const expectedTabs = [
      'tab-users', 'tab-appeals', 'tab-reports', 'tab-gifts',
      'tab-economy', 'tab-maintenance', 'tab-monitor', 'tab-banners',
      'tab-funfacts', 'tab-backups', 'tab-logs', 'tab-devices'
    ];
